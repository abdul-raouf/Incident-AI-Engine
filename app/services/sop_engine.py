...
- {{incident_time}}: Look for any time mention (e.g. "14:32", "2pm", "at noon")
...
If a placeholder's value truly cannot be found in the text, write "Not specified".
...
async def _fill_template(incident_text: str, category: IncidentCategory) -> str | None:
...
async def generate_sop(incident_text: str, classification: ClassificationOutput) -> str:
    if classification.primary_category == IncidentCategory.UNKNOWN:
        return "No SOP generated — incident could not be classified."
    ...
    import asyncio
    sop_tasks  = [_fill_template(incident_text, cat) for cat in active_categories]
